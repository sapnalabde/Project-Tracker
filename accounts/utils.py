...
#     usage = None#UserUsage(userusage=userdetail, product=product, quantity=rows, price=price)
#     usage.save()
...
def get_single_report_user(user):
    parentuser = get_parentuser(user)
    subuser = UserProfile.objects.filter(parentuser=parentuser, user__is_active=True)
    sub = [user]
    for userprofile in subuser:
        if user.userdetail.parentuser is None:
            if userprofile.userrole >= user.userdetail.userrole:
                sub.append(userprofile.user)
        else:
            if userprofile.userrole > user.userdetail.userrole:
                sub.append(userprofile.user)
    return sub